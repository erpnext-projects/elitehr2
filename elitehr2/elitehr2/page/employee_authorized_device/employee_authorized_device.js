...
frappe.pages['employee-authorized-device'].on_page_load = function (wrapper) {
	var page = frappe.ui.make_app_page({
		parent: wrapper,
		title: 'Employee Authorized Device',
		single_column: true
	});
	...
	$(`
		<h2>
			<svg class="icon text-ink-gray-7 current-color icon-sm" stroke="currentColor" style="" aria-hidden="true">
				<use class="" href="#icon-smartphone">
				</use>
			</svg>
			${__("Employee Authorized Device")}
		</h2>
		`).appendTo(wrapperContent);
	$(`<span>${__("Managing and reviewing employee authorized device requests")}</span><br>`).appendTo(wrapperContent);

	cardRow = $('<div class="cardContainers"></div>').appendTo(wrapperContent);
	tableContainer = $('<div class="requests-table" dir="rtl"></div>').appendTo(wrapperContent);
...
function loadRequests() {
	frappe.call({
		...
		args: { request_type: "ADD_AUTHORIZED_DEVICE" },
		callback: function (r) {
...
function renderTable(requests) {
	...
	const columns = [
		...
		{ id: "department", name: "device_name" },
