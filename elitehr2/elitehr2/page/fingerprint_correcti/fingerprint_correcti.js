let cardRow;
let tableContainer;
frappe.pages['fingerprint-correcti'].on_page_load = function (wrapper) {
	var page = frappe.ui.make_app_page({
		parent: wrapper,
		title: 'Fingerprint correction requests',
		single_column: true
	});
	...
	$(`
		<h2>
			<svg class="icon text-ink-gray-7 current-color icon-sm" stroke="currentColor" style="" aria-hidden="true">
				<use class="" href="#icon-file-text">
				</use>
			</svg>
			${__("Fingerprint correction requests")}
		</h2>
		`).appendTo(wrapperContent);
	$(`<span>${__("Managing and reviewing attendance and departure correction requests")}</span><br>`).appendTo(wrapperContent);
...
function loadRequests() {
	frappe.call({
		...
		args: { request_type: "ATTENDANCE_EDIT" },
		callback: function (r) {
