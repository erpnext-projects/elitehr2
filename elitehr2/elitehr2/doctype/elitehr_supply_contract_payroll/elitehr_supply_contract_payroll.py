...
class ElitehrSupplyContractPayroll(Document):
	def validate(self):
		...
		if self.hourly_wage == 0:
			frappe.throw(_("The selected contract does not have an hourly rate defined"))
	...
	def calc_payroll(self):
		...
		for cw in contract_workers:
			grouped[cw.name] = {
				"worker": cw.name,
    			"worker_name": cw.get("full_name") or "",
				"days": set(),
				"total_seconds": 0
			}
		...
		rows = frappe.get_all(
				...
				filters={
					...
					"date": ["between", [self.get("from"), self.get("to")]],
					"status": "Approved"
				},
				fields=["*"]
			)
		...
		for item in rows:
			...
			if worker in grouped:
				...
				grouped[worker]["total_seconds"] += flt(item.get("working_seconds"))
		...
		for row in grouped.values():      
			...
			total = total_hours_decimal * float(self.hourly_wage)
		...
		self.workers = len(worker_list)
		self.total_seconds = total_seconds
		self.hours = f"{str(total_hours).zfill(2)}:{str(total_minutes).zfill(2)}"
		self.total = flt(total_total, 2)
		self.net = flt(total_net, 2)
		self.total_deductions = flt(total_deductions, 2)
		
		# payroll
		self.table_data = {"data": payroll}
